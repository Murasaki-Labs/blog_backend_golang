...
run: ## helper for debug running
	...
	@env $$(cat .env | xargs) out/bin/blog-backend run
.PHONY: run
...
docker-arm: ## Builds the Docker image for ARM architecture
	docker buildx build --platform linux/arm64 --cache-to type=inline -t $(DOCKER_REPO):$(DOCKER_TAG) .

docker-amd: ## Builds the Docker image for AMD architectures
	docker buildx build --platform linux/amd64 --cache-to type=inline -t $(DOCKER_REPO):$(DOCKER_TAG) .
